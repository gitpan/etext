...
static char sccsid[] = "@(#) tkTextMark.c 1.11 95/06/28 10:42:39";
				 ...
				 * LangString(args[1]) is "mark". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " mark option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[2])[0];
    length = strlen(LangString(args[2]));
    if ((c == 'g') && (strncmp(LangString(args[2]), "gravity", length) == 0)) {
	if (argc > 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " mark gravity markName ?gravity?",
		             NULL);
	    return TCL_ERROR;
	}
	...
	if ((c == 'l') && (strncmp(LangString(args[4]), "left", length) == 0)) {
	    newTypePtr = &tkTextLeftMarkType;
	} else if ((c == 'r') && (strncmp(LangString(args[4]), "right", length) == 0)) {
	    newTypePtr = &tkTextRightMarkType;
	} else {
	    Tcl_AppendResult(interp, "bad mark gravity \"",
		    LangString(args[4]), "\": must be left or right",          NULL);
	    return TCL_ERROR;
	}
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " mark names\"",          NULL);
	    return TCL_ERROR;
	}
    ...
    } else if ((c == 'n') && (strncmp(LangString(args[2]), "next", length) == 0)) {
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " mark next index\"",          NULL);
	    return TCL_ERROR;
	}
	return MarkFindNext(interp, textPtr, LangString(args[3]));
    } else if ((c == 'p') && (strncmp(LangString(args[2]), "previous", length) == 0)) {
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " mark previous index\"",          NULL);
	    return TCL_ERROR;
	}
	return MarkFindPrev(interp, textPtr, LangString(args[3]));
    } else if ((c == 's') && (strncmp(LangString(args[2]), "set", length) == 0)) {
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " mark set markName index\"",          NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, LangString(args[4]), &index) != TCL_OK) {
	    return TCL_ERROR;
	}
	TkTextSetMark(textPtr, LangString(args[3]), &index);
    } else if ((c == 'u') && (strncmp(LangString(args[2]), "unset", length) == 0)) {
	for (i = 3; i < argc; i++) {
	...
	Tcl_AppendResult(interp, "bad mark option \"", LangString(args[2]),
 ...
 *
 *	Given a segment that is a mark, create an external index that
    ...
    TkTextIndex *indexPtr;	/* Index information gets stored here.  */
{
    ...
    TkTextIndexToExternalIndex(textPtr->tree,indexPtr,indexPtr);
}

/*
 *--------------------------------------------------------------
    ...
    char *string;			/* The starting index or mark name */
{
	...
	TkTextIndexToInternalSeg(&index, &index, NULL);
	segPtr = segPtr->nextPtr;
    } else {
	/*
	...
	for ( ; segPtr != NULL ; segPtr = segPtr->nextPtr) {
	    if (segPtr->typePtr == &tkTextRightMarkType ||
		    segPtr->typePtr == &tkTextLeftMarkType) {
		      found:
		Tcl_SetResult(interp, Tcl_GetHashKey(&textPtr->markTable, segPtr->body.mark.hPtr), TCL_STATIC);


		return TCL_OK;
	    } else if ((segPtr->typePtr == &tkTextIBTreeType)
		       && segPtr->body.itree.tree) {
		/* Go into internal tree. */
		index.linePtr = TkIBTreeFirstLine(segPtr);
	...
	if (index.linePtr == (TkTextLine *) NULL) {
	    /* Pop to external tree. */
	    if ( (segPtr = TkIBTreeNode2seg(linePtr->parentPtr, 
					    &(index.linePtr))) ) {
		segPtr = segPtr->nextPtr;
		continue;
	    }
    ...
    char *string;			/* The starting index or mark name */
{
    TkTextIndex index;
    Tcl_HashEntry *hPtr;
    register TkTextSegment *segPtr, *seg2Ptr, *prevPtr, *prevPtr2 = NULL;
    int offset, reached = 0;
	 ...
	 *
	 * prevPtr2 is the previous candidate, it is non-NULL only if
	 * we go forward into iBTree.
	 */
	for (prevPtr = prevPtr2, seg2Ptr = index.linePtr->segPtr; 
		...
		prevPtr = seg2Ptr;
	    } else if ((seg2Ptr->typePtr == &tkTextIBTreeType)
		       && seg2Ptr->body.itree.tree) {
		index.linePtr = TkIBTreeFirstLine(seg2Ptr);
		seg2Ptr = index.linePtr->segPtr;
	    }
	}
	if (prevPtr != NULL && seg2Ptr == segPtr) {
	    Tcl_SetResult(interp, Tcl_GetHashKey(&textPtr->markTable, prevPtr->body.mark.hPtr), TCL_STATIC);
