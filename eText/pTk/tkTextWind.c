...
static int		EmbWinConfigure _ANSI_ARGS_((TkText *textPtr,
			    TkTextSegment *ewPtr, int argc, Arg *args));
...
static Tk_ConfigSpec configSpecs[] = {
    {TK_CONFIG_CUSTOM, "-align",          NULL,          NULL,
	"center", 0, TK_CONFIG_DONT_SET_DEFAULT, &alignOption},
    {TK_CONFIG_CALLBACK, "-create",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextEmbWindow, create),
	TK_CONFIG_DONT_SET_DEFAULT|TK_CONFIG_NULL_OK},
    {TK_CONFIG_INT, "-padx",          NULL,          NULL,
	"0", Tk_Offset(TkTextEmbWindow, padX),
	TK_CONFIG_DONT_SET_DEFAULT},
    {TK_CONFIG_INT, "-pady",          NULL,          NULL,
	"0", Tk_Offset(TkTextEmbWindow, padY),
	TK_CONFIG_DONT_SET_DEFAULT},
    {TK_CONFIG_BOOLEAN, "-stretch",          NULL,          NULL,
	"0", Tk_Offset(TkTextEmbWindow, stretch),
	TK_CONFIG_DONT_SET_DEFAULT},
    {TK_CONFIG_WINDOW, "-window",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextEmbWindow, tkwin),
	TK_CONFIG_DONT_SET_DEFAULT|TK_CONFIG_NULL_OK},
				 ...
				 * LangString(args[1]) is "window". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " window option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[2])[0];
    length = strlen(LangString(args[2]));
    if ((strncmp(LangString(args[2]), "cget", length) == 0) && (length >= 2)) {
	TkTextIndex index;
	TkTextSegment *ewPtr;

	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " window cget index option\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, LangString(args[3]), &index) != TCL_OK) {
	    return TCL_ERROR;
	}
	...
	return Tk_ConfigureValue(interp, textPtr->tkwin, configSpecs,
		(char *) &ewPtr->body.ew, LangString(args[4]), 0);
    } else if ((strncmp(LangString(args[2]), "configure", length) == 0) && (length >= 2)) {
	TkTextIndex index;
	TkTextSegment *ewPtr;

	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " window configure index ?option value ...?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, LangString(args[3]), &index) != TCL_OK) {
	...
	if (argc == 4) {
	    return Tk_ConfigureInfo(interp, textPtr->tkwin, configSpecs,
		    (char *) &ewPtr->body.ew,          NULL, 0);
	} else if (argc == 5) {
	    return Tk_ConfigureInfo(interp, textPtr->tkwin, configSpecs,
		    (char *) &ewPtr->body.ew, LangString(args[4]), 0);
	} else {
	    TkTextChanged(textPtr, &index, &index);
	    return EmbWinConfigure(textPtr, ewPtr, argc-4, args+4);
	}
    } else if ((strncmp(LangString(args[2]), "create", length) == 0) && (length >= 2)) {
	TkTextIndex index;
	int lineIndex;
	...
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " window create index ?option value ...?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (TkTextGetIndex(interp, textPtr, LangString(args[3]), &index) != TCL_OK) {
	...
	if (EmbWinConfigure(textPtr, ewPtr, argc-4, args+4) != TCL_OK) {
	    TkTextIndex index2;

	    TkTextIndexForwChars(&index, 1, &index2);
	    TkBTreeDeleteChars(&index, &index2, 1, 1);
	    return TCL_ERROR;
	}
    } else if (strncmp(LangString(args[2]), "names", length) == 0) {
	Tcl_HashSearch search;
	Tcl_HashEntry *hPtr;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " window names\"",          NULL);
	    return TCL_ERROR;
	}
	...
	Tcl_AppendResult(interp, "bad window option \"", LangString(args[2]),
 ...
 *	for an embedded window, using an argc/args list.
 *
 * Results:
...
static int
EmbWinConfigure(textPtr, ewPtr, argc, args)
    TkText *textPtr;		/* Information about text widget that
				 * contains embedded window. */
    TkTextSegment *ewPtr;	/* Embedded window to be configured. */
    int argc;			/* Number of strings in args. */
    Arg *args;		/* Array of strings describing configuration
				 * options. */
{
    Tk_Window oldWindow;
    Tcl_HashEntry *hPtr;
    ...
    if (Tk_ConfigureWidget(textPtr->interp, textPtr->tkwin, configSpecs,
	    argc, args, (char *) &ewPtr->body.ew, TK_CONFIG_ARGV_ONLY)
	    != TCL_OK) {
	return TCL_ERROR;
    }
...
AlignParseProc(clientData, interp, tkwin, arg, widgRec, offset)
    ...
    Arg arg;				/* Value of option. */
    char *widgRec;			/* Pointer to TkTextEmbWindow
					 * structure. */
    int offset;				/* Offset into item (ignored). */
{
    char *value = LangString(arg);
				 ...
				 * been set by the caller. */
{
	...
	Tcl_DStringAppend(&name, Tcl_GetResult(textPtr->interp), -1);
