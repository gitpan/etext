...
static char rcsid[] = "$Id: tkTextTag.c,v 1.7 1996/03/12 00:09:47 ilya Exp $";
...
static Tk_ConfigSpec tagConfigSpecs[] = {
    {TK_CONFIG_BORDER, "-background",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, border), TK_CONFIG_NULL_OK},
    {TK_CONFIG_BITMAP, "-bgstipple",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, bgStipple), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-borderwidth",          NULL,          NULL,
	"0", Tk_Offset(TkTextTag, bdString),
		TK_CONFIG_DONT_SET_DEFAULT|TK_CONFIG_NULL_OK},
    {TK_CONFIG_BITMAP, "-fgstipple",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, fgStipple), TK_CONFIG_NULL_OK},
    {TK_CONFIG_FONT, "-font",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, fontPtr), TK_CONFIG_NULL_OK},
    {TK_CONFIG_COLOR, "-foreground",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, fgColor), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-justify",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, justifyString), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-lmargin1",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, lMargin1String), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-lmargin2",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, lMargin2String), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-offset",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, offsetString), TK_CONFIG_NULL_OK},
    {TK_CONFIG_LANGARG, "-overstrike",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, overstrikeString),
	TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-relief",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, reliefString), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-rmargin",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, rMarginString), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-spacing1",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, spacing1String), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-spacing2",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, spacing2String), TK_CONFIG_NULL_OK},
    {TK_CONFIG_STRING, "-spacing3",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, spacing3String), TK_CONFIG_NULL_OK},
    {TK_CONFIG_LANGARG, "-tabs",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, tabString), TK_CONFIG_NULL_OK},
    {TK_CONFIG_LANGARG, "-underline",          NULL,          NULL,
	         NULL, Tk_Offset(TkTextTag, underlineString),
	TK_CONFIG_NULL_OK},
				 ...
				 * LangString(args[1]) is "tag". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " tag option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[2])[0];
    length = strlen(LangString(args[2]));
    if ((c == 'a') && (strncmp(LangString(args[2]), "add", length) == 0)) {
	fullOption = "add";
	addTag = 1;
	...
	if (argc < 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag ", fullOption,
		    " tagName index1 ?index2 index1 index2 ...?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = TkTextCreateTag(textPtr, LangString(args[3]));
	for (i = 4; i < argc; i += 2) {
	    if (TkTextGetIndex(interp, textPtr, LangString(args[i]), &index1) != TCL_OK) {
		return TCL_ERROR;
	    }
	    if (argc > (i+1)) {
		if (TkTextGetIndex(interp, textPtr, LangString(args[i+1]), &index2)
			!= TCL_OK) {
		    return TCL_ERROR;
		}
    ...
    } else if ((c == 'b') && (strncmp(LangString(args[2]), "bind", length) == 0)) {
	if ((argc < 4) || (argc > 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag bind tagName ?sequence? ?command?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = TkTextCreateTag(textPtr, LangString(args[3]));
	...
	if (argc == 6) {
	    ...
	    if (LangString(args[5])[0] == 0) {
		return Tk_DeleteBinding(interp, textPtr->bindingTable,
			(ClientData) tagPtr, LangString(args[4]));
	    }
	    if (LangString(args[5])[0] == '+') {
		args[5] = LangStringArg(LangString(args[5])+1);
		append = 1;
	    }
	    mask = Tk_CreateBinding(interp, textPtr->bindingTable,
		    (ClientData) tagPtr, LangString(args[4]), args[5], append);
	    if (mask == 0) {
		return TCL_ERROR;
	    }
    ...
    } else if ((c == 'c') && (strncmp(LangString(args[2]), "cget", length) == 0)
	    && (length >= 2)) {
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag cget tagName option\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, LangString(args[3]));
	if (tagPtr == NULL) {
	    return TCL_ERROR;
	}
	return Tk_ConfigureValue(interp, textPtr->tkwin, tagConfigSpecs,
		(char *) tagPtr, LangString(args[4]), 0);
    } else if ((c == 'c') && (strncmp(LangString(args[2]), "configure", length) == 0)
	    && (length >= 2)) {
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag configure tagName ?option? ?value? ",
		    "?option value ...?\"",          NULL);
	    return TCL_ERROR;
	}
	tagPtr = TkTextCreateTag(textPtr, LangString(args[3]));
	...
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag delete tagName tagName ...\"",
		             NULL);
	    return TCL_ERROR;
	}
	for (i = 3; i < argc; i++) {
	    hPtr = Tcl_FindHashEntry(&textPtr->tagTable, LangString(args[i]));
	    if (hPtr == NULL) {
		continue;
	    }
    ...
    } else if ((c == 'l') && (strncmp(LangString(args[2]), "lower", length) == 0)) {
	TkTextTag *tagPtr2;
	int prio;

	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag lower tagName ?belowThis?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, LangString(args[3]));
	...
	if (argc == 5) {
	    tagPtr2 = FindTag(interp, textPtr, LangString(args[4]));
	    if (tagPtr2 == NULL) {
		return TCL_ERROR;
	    }
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag names ?index?\"",
		             NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    ...
	    if (TkTextGetIndex(interp, textPtr, LangString(args[3]), &index1)
		    != TCL_OK) {
		return TCL_ERROR;
	    }
    ...
    } else if ((c == 'n') && (strncmp(LangString(args[2]), "nextrange", length) == 0)
	    && (length >= 2)) {
	...
	if ((argc != 5) && (argc != 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag nextrange tagName index1 ?index2?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, LangString(args[3]));
	if (tagPtr == NULL) {
	    return TCL_OK;
	}
	...
	if (argc == 5) {
	    index2 = last;
	} else if (TkTextGetIndex(interp, textPtr, LangString(args[5]), &index2)
		!= TCL_OK) {
	    return TCL_ERROR;
	}
	...
	gotStart:
	if (TkTextIndexCmp(&tSearch.extIndex, &index2) >= 0) {
	    return TCL_OK;
	}
	TkTextPrintIndex(&tSearch.extIndex, position);
	Tcl_AppendElement(interp, position);
	TkBTreeNextTag(&tSearch);
	TkTextPrintIndex(&tSearch.extIndex, position);
	Tcl_AppendElement(interp, position);
    } else if ((c == 'p') && (strncmp(LangString(args[2]), "prevrange", length) == 0)
	    && (length >= 2)) {
	TkTextSearch tSearch;
	...
	if ((argc != 5) && (argc != 6)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag prevrange tagName index1 ?index2?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, LangString(args[3]));
    ...
    } else if ((c == 'r') && (strncmp(LangString(args[2]), "raise", length) == 0)
	    && (length >= 3)) {
	TkTextTag *tagPtr2;
	int prio;
	...
	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag raise tagName ?aboveThis?\"",
		             NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag(interp, textPtr, LangString(args[3]));
	...
	TkTextRedrawTag(textPtr, (TkTextIndex *) NULL, (TkTextIndex *) NULL,
		tagPtr, 1);
    } else if ((c == 'r') && (strncmp(LangString(args[2]), "ranges", length) == 0)
	    && (length >= 3)) {
	TkTextSearch tSearch;
	char position[TK_POS_CHARS];
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " tag ranges tagName\"",          NULL);
	    return TCL_ERROR;
	}
	tagPtr = FindTag((Tcl_Interp *) NULL, textPtr, LangString(args[3]));
	if (tagPtr == NULL) {
	...
	Tcl_AppendResult(interp, "bad tag option \"", LangString(args[2]),
 ...
 *	error message is recorded in Tcl_GetResult(interp) unless interp
 *	is NULL.
 *
 * Side effects:
 *	None.
    ...
    register TkTextTag *tagPtr;		/* Tag being deleted. */
{
    ...
    if (tagPtr->overstrikeString != NULL) {
	LangFreeArg(tagPtr->overstrikeString, TCL_DYNAMIC);
    }
    if (tagPtr->rMarginString != NULL) {
    ...
    if (tagPtr->tabString != NULL) {
	LangFreeArg(tagPtr->tabString,TCL_DYNAMIC);
    }
    if (tagPtr->tabArrayPtr != NULL) {
	ckfree((char *) tagPtr->tabArrayPtr);
    }
