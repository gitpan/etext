...
static char rcsid[] = "$Id: tkTextDisp.c,v 1.34 1996/03/12 00:09:28 ilya Exp $";
...
typedef struct DLine {
    ...
    int x;			/* X-position at which line is supposed to
				 * be drawn relative to the start of
				 * area occupied by outermost Dline.
				 * Should be 0 outside of IBTree. */
    ...
    int maxX;			/* Length of the chunk that contains
				 * this line, or -1. */
 ...
 *				in the window the last time that the window
 *				was laid out.
 * INTERNAL_DLINE -		Non-zero means that this DLine is
 *				inside IBTree.
 * DONTSHOW_DLINE -		Non-zero means that this DLine is
 *				inside IBTree, but should remain invisible.
 * ADDNL_DLINE -		Non-zero means that this DLine is additional
 *				line inside IBTree
 */
...
#define INTERNAL_DLINE	16
#define DONTSHOW_DLINE	32
#define ADDNL_DLINE	64
...
typedef struct IBTreeInfo {
    int numLines;		/* Number of lines to display. */
    TkText *textPtr;		/* Used to draw DLines inside. */
    DLine **lines;		/* Array of chains of lines to display. */
    int x;			/* When x-coord of ambient DLine were
				 * equal to this number, x-coords of
				 * internal DLines were correct. */
    int y;			/* When y-coord of ambient DLine were
				 * equal to this number, y-coords of
				 * internal DLines were correct. */
    DLine **addDlPtrs;		/* Additional array of lines to display. */
    int numAddLines;		/* Length of the additional array. */
} IBTreeInfo;
...
/* This is the default placer for DLines inside IBTree */
Tk_LinePosition defaultPosition = {  TK_JUSTIFY_LEFT, 0, 0, 0, 0, 0, 0 };

#define IN_OUTERMOST NULL
#define NO_MAXX 0
...
static DLine *		LayoutDLine _ANSI_ARGS_((TkText *textPtr,
			    TkTextIndex *indexPtr, 
			    Tk_LinePosition *internal, int maxX));
...
static void		ScrollByLines _ANSI_ARGS_((TkText *textPtr,
			    int offset));
static int		IBTreeMeasureProc _ANSI_ARGS_((
			    TkTextDispChunk *chunkPtr, int x, int yy));
...
static void		TextInvalidateRegion _ANSI_ARGS_((TkText *textPtr,
			    TkRegion region));
static int		DLinePixelIndex(TkText *textPtr, int x, int y,
					DLine *dlPtr);
...
static DLine *
LayoutDLine(textPtr, indexPtr, internal, maxX)
    TkText *textPtr;		/* Overall information about text widget. */
    TkTextIndex *indexPtr;	/* Beginning of display line.  May not
				 * necessarily point to a character segment. */
    Tk_LinePosition *internal;	/* NULL or pointer to a positioning
				 * code if the line is in the IBTree. */
    int maxX;			/* First pixel out of allowed drawing
				 * area. Ignored unless internal!=0. */
{
    ...
    dlPtr->count = 0;
    dlPtr->y = 0;
    dlPtr->x = 0;			/* Will be corrected in the caller. */
    dlPtr->oldY = -1;
    dlPtr->maxX = -1;
    dlPtr->height = 0;
    dlPtr->baseline = 0;
    dlPtr->chunkPtr = NULL;
    dlPtr->nextPtr = NULL;
    dlPtr->flags = NEW_LAYOUT | (internal ? INTERNAL_DLINE: 0);
    ...
    breakChunkPtr = NULL;
    breakCharOffset = 0;
    justify = internal ? internal->justify : TK_JUSTIFY_LEFT;
    tabIndex = -1;
    tabChunkPtr = NULL;
    tabArrayPtr = NULL;
    rMargin = 0;
    ...
    if (dlPtr->index.charIndex == 0) {
      if (internal) {
	dlPtr->spaceAbove = internal->spacing1;
      } else {
	dlPtr->spaceAbove = sValuePtr->spacing1;
      }
    } else {
      if (internal) {
	dlPtr->spaceAbove = internal->spacing2 - internal->spacing2/2;
      } else {
    ...
    if (wholeLine) {
      if (internal) {
	dlPtr->spaceBelow = internal->spacing3;
      } else {
	dlPtr->spaceBelow = sValuePtr->spacing3;
      }
    } else {
      if (internal) {
	dlPtr->spaceBelow = internal->spacing2/2;
      } else {
...
#if 0
		Tcl_SetVar2(textPtr->interp, "tk_textRelayout",          NULL,
			string,
			TCL_GLOBAL_ONLY|TCL_APPEND_VALUE|TCL_LIST_ELEMENT);
#endif
	    }
	    newPtr = LayoutDLine(textPtr, &index, 
				 IN_OUTERMOST, NO_MAXX);
	    if (prevPtr == NULL) {
...
static void
DisplayDLine(textPtr, dlPtr, prevPtr, pixmap, hoffset, voffset)
    ...
    Pixmap pixmap;		/* Pixmap to use for double-buffering.
				 ...
				 * to hold line. */
    int hoffset;		/* Horizontal offset for drawing. Shows
				 * how far to the left the pixmap starts
				 * relative to the DLine. */
    int voffset;		/* Vertical offset for drawing. Shows
				 * the gap between the top of pixmap
				 * and the top of DLine. Should be 0
				 * for non-internal DLine. */
{
    ...
    /* We do not need clearing the background if DLine is in
     * IBTree. */
    if (!(dlPtr->flags & INTERNAL_DLINE)) {
      Tk_Fill3DRectangle(textPtr->tkwin, pixmap, textPtr->border, 0, voffset,
	    Tk_Width(textPtr->tkwin), dlPtr->height, 0, TK_RELIEF_FLAT);
      hoffset += dInfoPtr->x - dInfoPtr->curPixelOffset;
    }
	...
	for (chunkPtr = dlPtr->chunkPtr; (chunkPtr != NULL);
		chunkPtr = chunkPtr->nextPtr) {
	    x = chunkPtr->x + hoffset;
	    if (chunkPtr->displayProc == TkTextInsertDisplayProc) {
		(*chunkPtr->displayProc)(chunkPtr, x, dlPtr->spaceAbove + voffset,
			dlPtr->height - dlPtr->spaceAbove - dlPtr->spaceBelow,
...
static void
DisplayLineBackground(textPtr, dlPtr, prevPtr, pixmap, hoffset, voffset)
    ...
    Pixmap pixmap;		/* Pixmap to use for double-buffering.
				 ...
				 * the widget. */
    int hoffset;		/* Horizontal offset for drawing. */
    int voffset;		/* Vertical offset for drawing. */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    ...
    leftX = chunkPtr->x;
    if ((dlPtr->flags & INTERNAL_DLINE) && (maxX > dlPtr->maxX)) {
      maxX = dlPtr->maxX;		/* Do not use hoffset here! */
    }
	...
	if (chunkPtr->stylePtr->bgGC != None) {
	    XFillRectangle(display, pixmap, chunkPtr->stylePtr->bgGC,
		    leftX + xOffset, voffset, (unsigned int) (rightX - leftX),
		    (unsigned int) dlPtr->height);
	    if (sValuePtr->relief != TK_RELIEF_FLAT) {
		Tk_3DVerticalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
			leftX + xOffset, voffset, sValuePtr->borderWidth,
			dlPtr->height, 1, sValuePtr->relief);
		Tk_3DVerticalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
			rightX - sValuePtr->borderWidth + xOffset,
			voffset, sValuePtr->borderWidth, dlPtr->height, 0,
			sValuePtr->relief);
	    }
	}
	leftX = rightX;
	...
	if (rightX <= rightX2) {
		...
		if (!matchLeft && (sValuePtr->relief != TK_RELIEF_FLAT)) {
		    Tk_3DHorizontalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
			    leftX + xOffset, voffset, rightX - leftX,
			    sValuePtr->borderWidth, leftXIn, 1, 1,
			    sValuePtr->relief);
		}
		leftX = rightX;
		leftXIn = 1;
	...
	if (matchLeft && !matchRight) {
	    if (sValuePtr->relief != TK_RELIEF_FLAT) {
		Tk_3DVerticalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
			rightX2 - sValuePtr->borderWidth + xOffset, voffset,
		...
		&& (sValuePtr->relief != TK_RELIEF_FLAT)) {
	    Tk_3DVerticalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
		    rightX2 + xOffset, voffset, sValuePtr->borderWidth,
		    sValuePtr->borderWidth, 1, sValuePtr->relief);
	    Tk_3DHorizontalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
		    leftX + xOffset, voffset, 
	...
	if (rightX <= rightX2) {
		...
		if (!matchLeft && (sValuePtr->relief != TK_RELIEF_FLAT)) {
		    Tk_3DHorizontalBevel(textPtr->tkwin, pixmap, sValuePtr->border,
			    leftX + xOffset,
			    dlPtr->height - sValuePtr->borderWidth + voffset,
    ...
    ClientData clientData;	/* Information about widget. */
{
    ...
    if (tkTextDebug) {
#if 0
	Tcl_SetVar2(textPtr->interp, "tk_textRedraw",          NULL,
		"", TCL_GLOBAL_ONLY);
#endif
    }
...
/*		    damagedCopies++; */
	...
	if (tkTextDebug) {
#if 0
	    Tcl_SetVar2(interp, "tk_textRedraw",          NULL, "borders",
		    TCL_GLOBAL_ONLY|TCL_APPEND_VALUE|TCL_LIST_ELEMENT);
#endif
	}
	...
	for (prevPtr = NULL, dlPtr = textPtr->dInfoPtr->dLinePtr;
		...
		DisplayDLine(textPtr, dlPtr, prevPtr, pixmap, 0, 0);
		if (dInfoPtr->dLinesInvalidated) {
				 ...
				 * LangString(args[1]) is "see". */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " see index\"",          NULL);
	return TCL_ERROR;
    }
    if (TkTextGetIndex(interp, textPtr, LangString(args[2]), &index) != TCL_OK) {
	return TCL_ERROR;
    }
...
int
TkTextXviewCmd(textPtr, interp, argc, args)
    TkText *textPtr;		/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * LangString(args[1]) is "xview". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
...
int
TkTextYviewCmd(textPtr, interp, argc, args)
    TkText *textPtr;		/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * LangString(args[1]) is "yview". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
	...
	if ((switchLength >= 2)
	    ...
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " yview -pickplace lineNum|index\"",
			         NULL);
		return TCL_ERROR;
	    }
	}
    ...
    if ((argc == 3) || pickPlace) {
	if (Tcl_GetInt(interp, args[2+pickPlace], &lineNum) == TCL_OK) {
	...
	if (TkTextGetIndex(interp, textPtr, LangString(args[2+pickPlace]),
		&index) != TCL_OK) {
	    return TCL_ERROR;
	}
    ...
    /*
     * New syntax: dispatch based on LangString(args[2]).
     */
...
int
TkTextScanCmd(textPtr, interp, argc, args)
    register TkText *textPtr;	/* Information about text widget. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings.  Someone else has already
				 * parsed this command enough to know that
				 * LangString(args[1]) is "scan". */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    TkTextIndex index;
    ...
    if (argc != 5) {
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " scan mark|dragto x y\"",          NULL);
	return TCL_ERROR;
    }
    if (Tcl_GetInt(interp, args[3], &x) != TCL_OK) {
	return TCL_ERROR;
    }
    if (Tcl_GetInt(interp, args[4], &y) != TCL_OK) {
	return TCL_ERROR;
    }
    c = LangString(args[2])[0];
    length = strlen(LangString(args[2]));
    if ((c == 'd') && (strncmp(LangString(args[2]), "dragto", length) == 0)) {
	/*
    ...
    } else if ((c == 'm') && (strncmp(LangString(args[2]), "mark", length) == 0)) {
	dInfoPtr->scanMarkChar = dInfoPtr->newCharOffset;
	dInfoPtr->scanMarkX = x;
	dInfoPtr->scanTotalScroll = 0;
	dInfoPtr->scanMarkY = y;
    } else {
	Tcl_AppendResult(interp, "bad scan option \"", LangString(args[2]),
 ...
 *
 * Results:
 *	If report is zero, then Tcl_GetResult(interp) is filled in with
 ...
 *	edge.  If report is non-zero, then Tcl_GetResult(interp) isn't modified
...
GetXView(interp, textPtr, report)
    Tcl_Interp *interp;			/* If "report" is FALSE, string
					 ...
					 * stored in Tcl_GetResult(interp). */
    TkText *textPtr;			/* Information about text widget. */
					 ...
					 * scrollbar if it has changed. */
{
    ...
    code = LangDoCallback(interp, textPtr->xScrollCmd, 0, 2, " %g %g", first, last);
    if (code != TCL_OK) {
 ...
 *	report is non-zero, then Tcl_GetResult(interp) isn't modified directly,
					 ...
					 * scrollbar if it has changed. */
{
    ...
    code = LangDoCallback(interp, textPtr->yScrollCmd, 0, 2, " %g %g",first, last);
 ...
 *	the character in DLine closest to that location.
 *
 * Results:
 *	Offset of the closest to (x,y) character in DLine with respect
 * 	to the start of DLine.
 *
 * Side effects:
 *	None.
 *
...
int
DLinePixelIndex(textPtr, x, y, dlPtr)
    TkText *textPtr;		/* Widget record for text widget. */
    int x, y;			/* Pixel coordinates of point in the
				 * same coordinate system as
				 * chunkPrt->x, dlPtr->y. */
    DLine *dlPtr;		/* DLine to search. */
{
    register TkTextDispChunk *chunkPtr;
    int offset = 0;
	...
	offset += (*chunkPtr->measureProc)(chunkPtr, x, y - dlPtr->y);
    }
    return offset;
}

				 ...
				 * index of the character nearest to (x,y). */
{
    ...
    *indexPtr = dlPtr->index;
    ...
    indexPtr->charIndex += DLinePixelIndex(textPtr, x, y, dlPtr);
}

/*
 *----------------------------------------------------------------------
...
static int
CharMeasureProc(chunkPtr, x, y)
    TkTextDispChunk *chunkPtr;		/* Chunk containing desired coord. */
    int x;				/* X-coordinate, in same coordinate
					 * system as chunkPtr->x. */
    int y;				/* Y-coordinate, ignored. */
{
    CharInfo *ciPtr = (CharInfo *) chunkPtr->clientData;
    int endX;
 ...
 * IBTreeDisplayProc --
 *
 *	This procedure is called to display a IBTree chunk on
 *	the screen or in an off-screen pixmap.
 *
 * Results:
 *	None.
...
static void
IBTreeDisplayProc(chunkPtr, x, y, height, baseline, display, dst, screenY)
    TkTextDispChunk *chunkPtr;		/* Chunk that is to be drawn. */
    int x;				/* X-position in dst at which to
					 * draw this chunk (may differ from
					 * the x-position in the chunk because
    ...
    int screenY;			/* Y-coordinate in text window that
					 ...
					 * the chunk. */
{
    IBTreeInfo *biPtr = (IBTreeInfo *) chunkPtr->clientData;
    DLine *dlPtr, *prevPtr;
    int line, i;
    int screenYTopOfChunk = screenY + baseline - chunkPtr->minAscent;
    int screenYTopOfDst = screenY - y;
    ...
    if (biPtr->y != screenYTopOfChunk) {
        /* Correct the y-coordinates in the internal DLines to have a
	 * correct display of embedded windows. */
        int shift = screenYTopOfChunk - biPtr->y;
        
        for (i = 0; i < biPtr->numAddLines; i++) {
	    biPtr->addDlPtrs[i]->y += shift;
	}
	for (line = 0 ; line < biPtr->numLines ; line++) {
	    for (dlPtr = biPtr->lines[line];
		 dlPtr != NULL;
		 dlPtr = dlPtr->nextPtr) {
	        dlPtr->y += shift;
	    }
	}
	biPtr->y = screenYTopOfChunk;
    }

    for (i = 0; i < biPtr->numAddLines; i++) {
      DisplayDLine(biPtr->textPtr, biPtr->addDlPtrs[i], NULL, dst, 
		   biPtr->addDlPtrs[i]->x + x - biPtr->x,
		   biPtr->addDlPtrs[i]->y - screenYTopOfDst);
    }
    for (line = 0 ; line < biPtr->numLines ; line++) {
	for (prevPtr = NULL, dlPtr = biPtr->lines[line];
		dlPtr != NULL;
		prevPtr = dlPtr, dlPtr = dlPtr->nextPtr) {
	    if (!(dlPtr->flags & DONTSHOW_DLINE)) {
	      DisplayDLine(biPtr->textPtr, dlPtr, prevPtr, dst, 
			   dlPtr->x + x - biPtr->x,
			   dlPtr->y - screenYTopOfDst);
	    }
	    dlPtr->oldY = dlPtr->y;
	    dlPtr->flags &= ~NEW_LAYOUT;
	}
...
static void
IBTreeUndisplayProc(textPtr, chunkPtr)
    TkText *textPtr;			/* Overall information about text
					 * widget. */
    TkTextDispChunk *chunkPtr;		/* Chunk that is about to be freed. */
{
    ...
    for (line = 0 ; line < biPtr->numLines ; line++) {
	FreeDLines(textPtr, biPtr->lines[line], NULL, 0);
    }
    while (i < biPtr->numAddLines) {
      ckfree((char*)biPtr->addDlPtrs[i++]);
    }
    if (biPtr->addDlPtrs) ckfree((char*)biPtr->addDlPtrs);
    if (biPtr->lines) ckfree((char *) biPtr->lines);
    ckfree((char *) biPtr);
}

/*
 ...
 *
 *	This procedure is called to determine which character in
 *	an IBTree chunk lies over a given x,y-coordinates.
 *
 * Results:
 *	The return value is the index *within the chunk* of the
 *	character that covers the position given by "x" and "y".
 *
 * Side effects:
 *	None.
 *
...
static int
IBTreeMeasureProc(chunkPtr, x, y)
    TkTextDispChunk *chunkPtr;		/* Chunk containing desired coord. */
    int x;				/* X-coordinate, in same coordinate
					 * system as chunkPtr->x. */
    int y;				/* Y-coordinate, in the coordinate
					 * system where dlPtr->y of
					 * containing string is 0. */
{
    IBTreeInfo *biPtr = (IBTreeInfo *) chunkPtr->clientData;
    DLine *dlPtr, *bestPtr = NULL;
    int line, dist, bestdist = INT_MAX, xoff, yoff;
    int index = 1, bestindex = 1;	/* Dummy char in beginning,
					 * thus 1. */
    int outside;			/* Distance of the point to the
					 * outside the bounding
					 * rectangle. Negative means
					 * it is outside. */

    /* Correct y to correspond to the current value of biPtr->y. */
    y += biPtr->y;
    outside = x - chunkPtr->x;
    if (outside > chunkPtr->x + chunkPtr->width - x) {
      outside = chunkPtr->x + chunkPtr->width - x;
    }
    if (outside < 0) {
      goto found;
    }
    /* Find the nearest DLine. Distance is |xoff|+|yoff|. */

    for (line = 0 ; line < biPtr->numLines ; line++) {
      for (dlPtr = biPtr->lines[line];
	   dlPtr != NULL ; dlPtr = dlPtr->nextPtr) {
	if (!(dlPtr->flags & (ADDNL_DLINE | DONTSHOW_DLINE))) {
					/* Do not process hidden and
					   additional DLines */
	  yoff = y - dlPtr->y;
	  if (yoff < 0) {
	    yoff = -yoff;
	  } else if (yoff > dlPtr->height) {
	    yoff -= dlPtr->height;
	  } else {
	    yoff = 0;
	  }
	  xoff = x - dlPtr->x;		/* Keeps information on the
					 * starting x-position of the
					 * chunk. */
	  if (xoff < 0) {
	    xoff = -xoff;
	  } else if (xoff > dlPtr->length) {
	    xoff -= dlPtr->length;
	  } else {
	    xoff = 0;
	  }
  ...
  found:
    if (!bestPtr || (bestdist > outside)) {
      /* Report the start or the end of the block. Since it is harder
       * to get to the start, prefer it. */
      
      if (x - chunkPtr->x <= chunkPtr->x + chunkPtr->width - x) {
	return 0;
      } else {
	return chunkPtr->numChars;	/* Should we subtract 1 ????? */
      }
    }
    return bestindex + DLinePixelIndex(biPtr->textPtr, 
				       x - bestPtr->x, y, bestPtr);
}

/*
 *--------------------------------------------------------------
...
static void
IBTreeBboxProc(chunkPtr, index, y, lineHeight, baseline, xPtr, yPtr,
	widthPtr, heightPtr)
    TkTextDispChunk *chunkPtr;		/* Chunk containing desired char. */
    int index;				/* Index of desired character within
					 * the chunk. */
					 ...
					 * character, in pixels. */
{
    ...
    if (index) {
      /* Here we are asked about something real or inter-field
       * separator. */
      index--;
      for (line = 0; line < biPtr->numLines; line++) {
	for (dlPtr = biPtr->lines[line];
	     dlPtr != NULL ; dlPtr = dlPtr->nextPtr) {
	  if (index < dlPtr->count) {
	    goto found;
	  }
	  index -= dlPtr->count;
	}
    ...
    found:
      if (dlPtr == NULL) {
	panic("Ran out of DLines in IBTreeBboxProc.");
      }

      if (dlPtr->flags & DONTSHOW_DLINE) {
	/* Asked about a hidden line in a block */
	/* X and Y as for the whole chunk. */
	*xPtr = chunkPtr->x;
	*yPtr = y + baseline - chunkPtr->minAscent;
	*heightPtr = 0;
	*widthPtr = 0;
	return;
      ...
      if (index == dlPtr->count - 1) {
	/* Asked about a whole field in a block */
	*xPtr = dlPtr->x;
	*yPtr = dlPtr->y + y + baseline - chunkPtr->minAscent;
	*heightPtr = dlPtr->height;
	*widthPtr = dlPtr->length;	/* Contains width! */
      } else {
	int ascent = chunkPtr->minAscent;

	for (chunkPtr = dlPtr->chunkPtr; ; chunkPtr = chunkPtr->nextPtr) {
	  if (chunkPtr == NULL) {
	    panic("Ran out of chunks in IBTreeBboxProc");
	  }
	  if (index < chunkPtr->numChars) {
	    (*chunkPtr->bboxProc)(chunkPtr, index, 
		dlPtr->y + dlPtr->spaceAbove,
		dlPtr->height - dlPtr->spaceAbove - dlPtr->spaceBelow,
		dlPtr->baseline - dlPtr->spaceAbove, xPtr, yPtr, widthPtr,
		heightPtr);
	    *xPtr += dlPtr->x;
	    *yPtr += y + baseline - ascent;
	    return;
	  }
	  index -= chunkPtr->numChars;
	}
      ...
      /* Question about the whole chunk. */
      *xPtr = chunkPtr->x;
      *yPtr = y + baseline - chunkPtr->minAscent;
      *heightPtr = chunkPtr->minAscent + chunkPtr->minDescent;
      *widthPtr = chunkPtr->width;
    }
}

/*
 *--------------------------------------------------------------
 *
 * TkTextIBTreeLayoutProc --
 *
 *	This procedure is the "layoutProc" for IBTree segments.
 *
 * Results:
 *	If there is something to display for the chunk then a
 *	non-zero value is returned and the fields of chunkPtr
 ...
 * Side effects:
 *	Memory is allocated to hold additional information about
 *	the chunk. If a callback procedure is called. almost anything
 *	 can happen.
 *
 *--------------------------------------------------------------
 */
...
int
TkTextIBTreeLayoutProc(textPtr, indexPtr, segPtr, offset, maxX, maxChars,
	noCharsYet, wrapMode, chunkPtr)
    TkText *textPtr;		/* Text widget being layed out. */
    TkTextIndex *indexPtr;	/* Index of first character to lay out
				 * (corresponds to segPtr and offset). */
    ...
    register TkTextDispChunk *chunkPtr;
				 ...
				 * been set by the caller. */
{
    TkTextLine *linePtr = NULL, *nextPtr;
    IBTreeInfo *infoPtr = ckalloc(sizeof(IBTreeInfo));
    int line, y = 0, l = 0, ll = 0;
    TkTextIndex index;
    ...
    infoPtr->numLines = 0;		/* One fake line in the end! */
    infoPtr->textPtr = textPtr;
    nextPtr = linePtr;
    while (nextPtr && (nextPtr = TkBTreeNextLine(nextPtr))) {
      infoPtr->numLines++;
    }

    infoPtr->lines = infoPtr->numLines 
      ? ckalloc(infoPtr->numLines * sizeof(DLine *))
      : NULL;
    infoPtr->y = 0;
    infoPtr->x = chunkPtr->x;
    infoPtr->addDlPtrs = NULL;
    infoPtr->numAddLines = 0;

    index.tree = segPtr->body.itree.tree;

    for (line=0; line < infoPtr->numLines; 
	 line++, linePtr = TkBTreeNextLine(linePtr)) {
      DLine *dlPtr, *oldPtr = NULL;
      
      index.charIndex = 0;
      index.linePtr = linePtr;
      infoPtr->lines[line] = NULL;
      l = 0;
      do {
	dlPtr = LayoutDLine(textPtr, &index, 
			    &defaultPosition, maxX - chunkPtr->x);
	dlPtr->x = chunkPtr->x;
	if (infoPtr->lines[line]) {
	  oldPtr->nextPtr = dlPtr;
	} else {
	  infoPtr->lines[line] = dlPtr;
	}
	oldPtr = dlPtr;
	dlPtr->y = y;
	y += dlPtr->height;
	...
	dlPtr->nextPtr = NULL;
	TkTextIndexForwChars(&index, dlPtr->count, &index);
      } while (index.linePtr == linePtr);
      if (blockPtr->layoutCmd) {
	dlPtr = infoPtr->lines[line];
	while (dlPtr) {
	  dlPtr->maxX = l;
	  dlPtr = dlPtr->nextPtr;
	}
    ...
    if (blockPtr->layoutCmd) {
      ...
      ListFactory *listFactoryPtr = &listFactory;
      int result, argc, argc1, code, i, count;
      Arg *args, *args1, arg = NULL, widget = NULL;
      dArgBuffer;
      DLine **addDlPtrs, *addDlPtr;
      LangFreeProc *freeProc = NULL, *freeProc1 = NULL;
      Tcl_FreeProc *freeProc2 = NULL;

      ListFactoryInit(listFactoryPtr);
      arg = LangCallbackArg(blockPtr->layoutCmd);
      if (LangIsList(arg)) {
	ListFactoryAppendList(listFactoryPtr,arg);
      } else {
	ListFactoryAppendCopy(listFactoryPtr, arg);
      ...
      LangSetString(&arg, blockPtr->name);
      ListFactoryAppendCopy(listFactoryPtr, arg);
      widget = LangWidgetArg(textPtr->interp, textPtr->tkwin);
      ListFactoryAppendCopy(listFactoryPtr, widget);
      if (segPtr->body.itree.data) {
	ListFactoryAppendCopy(listFactoryPtr, segPtr->body.itree.data);
      }
      listFactoryPtr = ListFactoryEndLevel(listFactoryPtr);
      LangSetDefaultBuffer(&arg);
      LangSetInt(&arg, chunkPtr->x);
      ListFactoryAppendCopy(listFactoryPtr, arg);
      LangFreeArg(arg, LANG_DYNAMIC);
      TkIBTreePrint(segPtr, listFactoryPtr, (void**) infoPtr->lines);
      ListFactoryFinish(listFactoryPtr);
      result = LangArgEval(textPtr->interp, ListFactoryArg(listFactoryPtr));
      ListFactoryFree(listFactoryPtr);
      if (result != TCL_OK) {
	Tcl_AppendResult(textPtr->interp, 
			 ": Error during layoutBlock callback.",
			          NULL);
	Tcl_BackgroundError(textPtr->interp);
	goto finishCall;
      }
      line = 0;
      resultArg = Tcl_ResultArg(textPtr->interp);
      code = Lang_SplitList(textPtr->interp, resultArg,
			    &argc, &args, &freeProc);
      if ((code != TCL_OK) || (argc == 0)) {
	if (args != NULL && freeProc) (*freeProc)(argc,args);
	Tcl_AppendResult(textPtr->interp, 
			 ": Did not get list from LayoutProc.",          NULL);
	Tcl_BackgroundError(textPtr->interp);
	goto finishCall;
      }
      for (i = 0; i < argc; i++) {
	code = Lang_SplitList(textPtr->interp, args[i],
			      &argc1, &args1, &freeProc1);
	if ((code != TCL_OK) 
	    || !(((argc1 == 1) && (i != 0)) 
		 || (argc1 == 7) || (argc1 == 13))) {
	  if (args != NULL && freeProc) (*freeProc)(argc,args);
	  if (args1 != NULL && freeProc1) (*freeProc1)(argc1,args1);
	  Tcl_AppendResult(textPtr->interp, 
			   ((code == TCL_OK) ?
			    "List of wrong size as element in return from LayoutProc" :
			    ": Did not get list from LayoutProc."), 
			            NULL);
	  Tcl_BackgroundError(textPtr->interp);
	  goto finishCall;
	}
	count = LangInt(args1[0]);
	if (i == 0) {
	  chunkPtr->minAscent = LangInt(args1[6]);
	  chunkPtr->minDescent = LangInt(args1[5]) - LangInt(args1[6]);
	  chunkPtr->minHeight = 0;
	  chunkPtr->width = LangInt(args1[3]);
	} else if (line >= infoPtr->numLines && count) {
	  if (count > textPtr->numDLines) {
	    if (args != NULL && freeProc) (*freeProc)(argc,args);
	    if (args1 != NULL && freeProc1) (*freeProc1)(argc1,args1);
	    Tcl_AppendResult(textPtr->interp, "Ordinal of additional ",
			     "DLine too big for block\"", blockPtr->name,
			     "\".",          NULL);
	    Tcl_BackgroundError(textPtr->interp);
	    goto finishCall;
	  }
	  addDlPtr = (DLine*)ckalloc(sizeof(DLine));
	  memcpy((VOID *) addDlPtr, (VOID *) textPtr->dlinePtrs[count-1],
		 sizeof(DLine));
	  addDlPtr->x = LangInt(args1[1]) + chunkPtr->x;
	  addDlPtr->y = LangInt(args1[2]);
	  addDlPtr->length = LangInt(args1[3]);
	  addDlPtr->maxX = LangInt(args1[4]);
	  addDlPtr->height = LangInt(args1[5]);
	  addDlPtr->baseline = LangInt(args1[6]);
	  addDlPtr->flags |= INTERNAL_DLINE | ADDNL_DLINE;
	  addDlPtrs = infoPtr->addDlPtrs;
	  infoPtr->addDlPtrs = 
	    (DLine**)ckalloc((infoPtr->numAddLines+1)*sizeof(DLine*));
	  if (addDlPtrs) {
	    memcpy((VOID *) infoPtr->addDlPtrs, (VOID *)addDlPtrs,
		   infoPtr->numAddLines*sizeof(DLine*));
	    ckfree(addDlPtrs);
	  }
	  infoPtr->addDlPtrs[infoPtr->numAddLines++] = addDlPtr;
	} else if ((count != 0) && (argc1 == 1) 
		   && count <= infoPtr->numLines - line) {
	  while (count--) {
	    infoPtr->lines[line]->flags |= DONTSHOW_DLINE;
	    line++;
	  }
	} else if (count != 0
		   && count <= infoPtr->numLines - line) {
	  /* If it is 0, ignore it! */
	  int vshift = LangInt(args1[2]) - infoPtr->lines[line]->y;
	  int singleLine = (count == 1) 
	    && (infoPtr->lines[line]->nextPtr == NULL);
	  DLine *intLine;

	  while (count-- && line < infoPtr->numLines) {
	    intLine = infoPtr->lines[line];
	    while (intLine) {
	      intLine->x = LangInt(args1[1]) + chunkPtr->x;
	      intLine->y += vshift;
	      intLine->length = LangInt(args1[3]);
	      intLine->maxX = LangInt(args1[4]);
	      if (singleLine) {
		intLine->height = LangInt(args1[5]);
		intLine->baseline = LangInt(args1[6]);
	      }
	      intLine = intLine->nextPtr;
	    }
	    line++;
	  }
	} else if (count > infoPtr->numLines - line) {
	    if (args != NULL && freeProc) (*freeProc)(argc,args);
	    if (args1 != NULL && freeProc1) (*freeProc1)(argc1,args1);
	    Tcl_AppendResult(textPtr->interp, "Too big count of merged lines ",
			     "for block\"", blockPtr->name,
			     "\".",          NULL);
	    Tcl_BackgroundError(textPtr->interp);
	    goto finishCall;
	}
	if (args1 != NULL && freeProc1) (*freeProc1)(argc1,args1);
      ...
      if (args != NULL && freeProc) (*freeProc)(argc,args);
      if (line != infoPtr->numLines) {
	Tcl_AppendResult(textPtr->interp, "wrong # data returned during ",
			 "layoutBlock callback.",
			          NULL);
	Tcl_BackgroundError(textPtr->interp);
	goto finishCall;
      }
  ...
  finishCall:
      for (line=0; line < infoPtr->numLines; line++) {
	DLine *dlPtr = infoPtr->lines[line];
	while (dlPtr) {
	  dlPtr->maxX = ll;
	  dlPtr = dlPtr->nextPtr;
	}
      }
      chunkPtr->minAscent = (textPtr->fontPtr->ascent + y)/2;
      chunkPtr->minDescent = y - chunkPtr->minAscent;
      chunkPtr->minHeight = 0;
      chunkPtr->width = ll;
    }
    if (resultArg)  LangFreeArg(resultArg, LANG_DYNAMIC);
  
    /*
     * Fill in the chunk structure.
     */

    chunkPtr->displayProc = IBTreeDisplayProc;
    chunkPtr->undisplayProc = IBTreeUndisplayProc;
    chunkPtr->measureProc = IBTreeMeasureProc;
    chunkPtr->bboxProc = IBTreeBboxProc;
    chunkPtr->numChars = segPtr->size;
    chunkPtr->breakIndex = -1;
    chunkPtr->clientData = (ClientData) infoPtr;
    return 1;
}

/*
 ...
 * TkTextPrintDLines --
 *
 *	Appends description of several blocks of DLines to the listFactory.
 *
 * Results:
 *	None.
 *
...
void
TkTextPrintDLines(listFactoryPtr, dlPtrs, count)
     ListFactory *listFactoryPtr;	/* Where to print. */
     DLine **dlPtrs;			/* Which lines to report. */
     int count;				/* How many lines to report. */
{
  DLine *nextPtr;
  int h = 0, des = 0, n = 0, w = 0, ww = 0;
  char buffer[TCL_DOUBLE_SPACE];
  Arg arg = NULL;

  LangSetBuffer(&arg, buffer);
  LangSetInt(&arg, count);
  ListFactoryAppendCopy(listFactoryPtr, arg);
  LangSetInt(&arg, count ? dlPtrs[0]->y : 
	                   dlPtrs[-1]->y + dlPtrs[-1]->height);
  ListFactoryAppendCopy(listFactoryPtr, arg);
  nextPtr = dlPtrs[0];
  if (count) {
    while (nextPtr) {
      h += nextPtr->height;
      des = nextPtr->height - nextPtr->baseline;
      if (w < nextPtr->length) {
	w = nextPtr->length;
      }
      if (ww < nextPtr->maxX) {
...
DLine *
TkTextGetDLine(textPtr, indexPtr)
    TkText *textPtr;		/* Overall information about text widget. */
    TkTextIndex *indexPtr;	/* Beginning of display line.  May not
				 * necessarily point to a character segment. */
{
  return LayoutDLine(textPtr, indexPtr, IN_OUTERMOST, NO_MAXX);
}

/*
 *----------------------------------------------------------------------
 ...
 *
 *	This procedure is called to free up all of the resources
 *	associated with one DLine structure.
 *
 * Results:
 *	None.
 *
...
void
TkTextFreeDLine(textPtr, dlPtr)
    TkText *textPtr;			/* Information about overall text
					 * widget. */
    register DLine *dlPtr;		/* Pointer to first DLine to free up. */
{
  FreeDLines(textPtr, dlPtr, NULL, 0);
}

/*
 *----------------------------------------------------------------------
